...
{
public:
    Vector3f v0, v1, v2; //三角形三个顶点的位置点坐标，按逆时针顺序
    Vector3f e1, e2;     //边向量：v1-v0, v2-v0，用于计算数据成员normal
    Vector3f t0, t1, t2; //纹理坐标texture coords
    Vector3f normal;     //三角形面法向量方向
    Material* m;         //材质类型
...
inline Intersection Triangle::getIntersection(Ray ray)
{
    ...
    if (t_tmp<0)    return inter; // no intersection
    // intersection--assgin other information
    inter.distance = t_tmp; // time needed to intersect
    inter.obj=this;
    inter.happened = true;
    inter.normal=normal;
    inter.m=m;
    inter.coords=ray(t_tmp); // coords=origin+t_tmp*direction
    return inter;
}
