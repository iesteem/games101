...
void naive_bezier(const std::vector<cv::Point2f> &points, cv::Mat &window) 
{
    ...
    for (double t = 0.0; t <= 1.0; t += 0.001)//对t在[0,1]范围内，每次递增0.001
    {
        auto point = std::pow(1 - t, 3) * p_0 + 3 * t * std::pow(1 - t, 2) * p_1 +
                 3 * std::pow(t, 2) * (1 - t) * p_2 + std::pow(t, 3) * p_3;//带入课程公式
...
cv::Point2f recursive_bezier(const std::vector<cv::Point2f> &control_points, float t) 
{
    ...
    for(int i = 0; i < control_points.size() - 1; i++)//这里需要control_points.size() - 1才能保证仅剩余一个控制点
        points.push_back( t * control_points[i] + (1 - t)*control_points[ i + 1]);//系数是不是错了？
...
void bezier(const std::vector<cv::Point2f> &control_points, cv::Mat &window) 
{
    ...
    for(double t = 0.0; t <= 1.0; t += 0.0001)//对t在[0,1]范围内，每次递增0.0001
    {
        auto point = recursive_bezier(control_points, t);//调用recursive_bezier函数，在当前t取值时找到唯一point点
